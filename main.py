...
def download_file():
    ...
    try:
        return send_from_directory(app.config['UPLOAD_FOLDER'], path=output_path, as_attachment=True)
...
def translate():
    ...
    if lang == "":
        ...
        return jsonify({"error": "output language is required."})
    ...
    file.save(file_path)
    thread = threading.Thread(target=translateTask, args=(file_path,lang,))
    thread.start()
...
def transcribe():
    ...
    file_path = os.path.join(app.config['UPLOAD_FOLDER'], file.filename)
    file.save(file_path)
    thread = threading.Thread(target=transcribeTask, args=(file_path, lang, ))
    thread.start()
    return jsonify({"message": "file saved", "path": file_path, "task": "transcription created"}, 200)

def translateTask(file_path: str, output_lang: str) -> None:
...
def transcribeTask(file_path: str, output_lang:str) -> None:
    ...
    try:
        ...
        logger.info("notification sent, no waiting for response.")
    logger.info(f"transcription completed for {file_path} from {audio_lang} to {output_lang} saved in {output_path}.")
